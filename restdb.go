...
func FindUserID(ID int) User {
	...
	for rows.Next() {
		err = rows.Scan(&u.ID, &u.Username, &u.Password, &u.LastLogin, &u.Admin, &u.Active)
		if err != nil {
			log.Println(err)
			return User{}
		}
...
func IsUserValid(u User) bool {
	...
	rows, err := db.Query("SELECT Password FROM users WHERE Username = $1 \n", u.Username)
	if err != nil {
		log.Println(err)
		return false
	}
	...
	// we will get the FIRST ONE only.
	for rows.Next() {
		err = rows.Scan(&tempPassword)
		if err != nil {
			log.Println(err)
			return false
		}
		if u.Password == tempPassword {
			return true
		}
